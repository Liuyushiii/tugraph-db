...
class KeyPacker {
    ...
    // new append
    // 版本偏移量 (0 + VID_SIZE + 1)
    static const size_t VER_OFF = PT_OFF + 1;
    // 标签偏移量 (0 + VID_SIZE + 1 + VERSION_SIZE)
    static const size_t LID_OFF = VER_OFF + ::lgraph::_detail::VID_SIZE;            
                               // label

    // static const size_t LID_OFF = PT_OFF + 1;    
    // primary id 偏移量
    ...
    // new append
    // version 偏移量
    // static const size_t VER_OFF = EID_OFF + ::lgraph::_detail::EID_SIZE;            // version id
    // edge key大小
    static const size_t EDGE_KEY_SIZE = EID_OFF + ::lgraph::_detail::EID_SIZE;
    // new append （替换了上面的方法）
    // static const size_t EDGE_KEY_SIZE = VER_OFF + ::lgraph::_detail::VID_SIZE;
 ...
 public:
    ...
    static VertexId GetVersion(const Value& v) {
        return GetNByteIntId<::lgraph::_detail::VID_SIZE>(v.Data() + VER_OFF);
    }
    ...
    static Value CreateEdgeKey(PackType et, EdgeUid euid) {
        FMA_LOG() << "CreateEdgeKey is invoked, uid: " << euid.ToString();
        Value v(EDGE_KEY_SIZE);
        SetNByteIntId<::lgraph::_detail::VID_SIZE>(v.Data(), euid.src);
        SetPackType(v.Data() + PT_OFF, et);
        // new append 待改
        // SetNByteIntId<::lgraph::_detail::VERSION_SIZE>(v.Data() + VERSION_OFF, (int64_t)0);
    ...
    static Value CreateEdgeKeyVersion(PackType et, EdgeUid euid) {
        ...
        // SetNByteIntId<::lgraph::_detail::EID_SIZE>(v.Data() + EID_OFF+::lgraph::_detail::VID_SIZE, euid.eid);
        // SetNByteIntId<::lgraph::_detail::VID_SIZE>(v.Data() + VER_OFF+::lgraph::_detail::VID_SIZE, euid.verid);
        SetNByteIntId<::lgraph::_detail::VID_SIZE>(v.Data() + VER_OFF, euid.verid);
        SetNByteIntId<::lgraph::_detail::LID_SIZE>(v.Data() + LID_OFF, euid.lid);
        SetNByteIntId<::lgraph::_detail::TID_SIZE>(v.Data() + TID_OFF, euid.tid);
        SetNByteIntId<::lgraph::_detail::VID_SIZE>(v.Data() + SID_OFF, euid.dst);
    ...
    static Value CreateInEdgeKeyVersion(EdgeUid euid) {
        FMA_LOG() << "CreateInEdgeKeyVersion: " << euid.ToString();
        return CreateEdgeKeyVersion(PackType::IN_EDGE, euid);
    }
    /**
    ...
    static Value CreateOutEdgeKeyVersion(EdgeUid euid) {
        // return CreateEdgeKey(PackType::OUT_EDGE, euid);
        FMA_LOG() << "CreateOutEdgeKeyVersion: " << euid.ToString();
        return CreateEdgeKeyVersion(PackType::OUT_EDGE, euid);
    }
...
class EdgeValue {
 ...
 public:
    ...
    struct SizeIndicator {
        uint16_t lid_size : 2;
        uint16_t tid_indicator_size : 1;
        uint16_t vid_size : 3;
        uint16_t eid_size : 2;
        // need to be deleted
        // uint16_t test_size : 8;

        SizeIndicator() = default;
        SizeIndicator(size_t l, size_t p, size_t v, size_t e)
            : lid_size((uint16_t)l), vid_size((uint16_t)v), eid_size((uint16_t)e) {
            tid_indicator_size = p == 0 ? 0 : 1;
        }
        operator uint16_t() const { return *(uint16_t*)this; }

        uint16_t PidSize() const { return tid_indicator_size == 0 ? 0 : 8; }

        uint16_t TotalSize() const { return lid_size + PidSize() + vid_size + eid_size; }
    };

    // new append
    struct SizeIndicatorVersion {
        uint16_t lid_size : 2;
        uint16_t tid_indicator_size : 1;
        uint16_t vid_size : 3;
        uint16_t eid_size : 2;
        ...
        SizeIndicatorVersion() = default;
        SizeIndicatorVersion(size_t l, size_t p, size_t v, size_t e, size_t version)
            : lid_size((uint16_t)l), vid_size((uint16_t)v), eid_size((uint16_t)e), ver_size((uint16_t)version){
            tid_indicator_size = p == 0 ? 0 : 1;
        }
        operator uint16_t() const { return *(uint16_t*)this; }
        ...
        uint16_t TotalSize() const { return lid_size + PidSize() + vid_size + eid_size + ver_size; }
    };
    ...
    static inline size_t GetHeaderSize(const char* p) {
        return ((SizeIndicator*)p)->TotalSize() + 2;
    }
    ...
    // new append
    static size_t GetHeaderSizeRequiredVersion(LabelId lid, TemporalId tid, VertexId vid, EdgeId eid, VertexId version) {
        return (size_t)2  // indicator
               + GetLidSizeRequired(lid) + GetPidSizeRequired(tid) + GetVidSizeRequired(vid) +
               GetEidSizeRequired(eid) + GetVidSizeRequired(version) ;
    }
    /**
    ...
    // new append
    static const char* ParseHeaderVersion(const char* p, LabelId& lid, TemporalId& tid, VertexId& vid,
                                   EdgeId& eid, VertexId& version) {
        SizeIndicatorVersion indicator = *(SizeIndicatorVersion*)p;
        p+=2; // heihei
        // get label id
        size_t lid_size = indicator.lid_size;
        FMA_DBG_ASSERT(lid_size <= 2);
    ...
    // new append
    static char* SetHeaderVersion(char* p, LabelId lid, TemporalId tid, VertexId vid, EdgeId eid, VertexId version) {
        // get indicator
        size_t lid_size = GetLidSizeRequired(lid);
        size_t tid_size = GetPidSizeRequired(tid);
        size_t vid_size = GetVidSizeRequired(vid);
        size_t eid_size = GetEidSizeRequired(eid);
        size_t ver_size = GetVidSizeRequired(version);
        // FMA_LOG() << "lid_size: " << lid_size << ", tid_size: " << tid_size << ", vid_size: " << vid_size << ", eid_size: " << eid_size << ", ver_size: " << ver_size;
        SizeIndicatorVersion indicator(lid_size, tid_size, vid_size, eid_size, ver_size);
        *(uint16_t*)p = indicator;
        p+=2; // heihei
        // version
        // if (ver_size != 0) {
        //     ::lgraph::_detail::SetNByteIdToBuf(p, ver_size, version);
        //     p += ver_size;
        // }
        // label id
        if (lid_size != 0) {
 ...
 public:
    ...
    // beg is a sequential iterator of std::tuple<LabelId, TemporalId, VertexId, DenseString>
    
    EdgeValue(const IT& beg,  
        ...
        for (next_begin = beg; next_begin != end; next_begin++) {
            ...
            // new append
            VertexId version = std::get<4>(*next_begin);
            // FMA_LOG() << "version1: " << version;
            ...
            // new append (替换了上面的代码)
            hsize = GetHeaderSizeRequiredVersion(lid, tid, vid, eid, version);
        ...
        for (auto it = beg; it != next_begin; ++it) {
            ...
            // new append
            VertexId version = std::get<4>(*it);
            // FMA_LOG() << "version2: " << version;
            ...
            // size_t before_set = buf - v_.Data();

            // new append（替换了上面的代码）
            // 只有同起点、终点的边才会有不同的eid，否则都是0
            buf = SetHeaderVersion(buf, lid, tid, vid, last_eid, version);

            // size_t after_set = buf - v_.Data();
            // FMA_LOG() << "size of header: " << after_set - before_set;
    ...
    void ParseNthEdgeVersion(size_t n, LabelId& lid, TemporalId& tid, VertexId& vid, EdgeId& eid, VertexId& version,
                      const char*& prop, size_t& prop_size) const {
        const char* p = GetNthEdge(n);
        prop = ParseHeaderVersion(p, lid, tid, vid, eid, version);
        prop_size = GetNthEdge(n + 1) - prop;
    }
    /**
     * Gets nth edge data
    ...
    EdgeData GetNthEdgeDataVersion(size_t n) const {
        EdgeData ed;
        ParseNthEdgeVersion(n, ed.lid, ed.tid, ed.vid, ed.eid, ed.version, ed.prop, ed.psize);
        return ed;
    }
    ...
    // new append
    size_t SearchEdgeVersion(LabelId lid, TemporalId tid, VertexId vid, EdgeId eid, VertexId version, bool& found) const {
        found = false;
        size_t beg = 0;
        size_t end = n_;
        while (beg < end) {
    ...
    Value CreateOutEdgeKey(VertexId src) const {
        ...
        // new append (替换了上面的代码)
        ParseHeaderVersion(e, lid, tid, vid, eid, version_id);
        FMA_LOG() << "parsed version in createOutEdge: " << version_id;
        EdgeUid euid = EdgeUid(src, vid, lid, tid, eid);
        euid.verid = version_id;
        return KeyPacker::CreateOutEdgeKeyVersion(euid);
    }
    ...
    Value CreateInEdgeKey(VertexId dst) const {
        ...
        // new append (替换了上面的代码)
        ParseHeaderVersion(e, lid, tid, vid, eid, version_id);
        FMA_LOG() << "parsed version in createInEdge: " << version_id;
        EdgeUid euid = EdgeUid(dst, vid, lid, tid, eid);
        euid.verid = version_id;
        return KeyPacker::CreateInEdgeKeyVersion(euid);
    }
